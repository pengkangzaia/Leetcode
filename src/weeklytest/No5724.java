...
public class No5724 {
    ...
    public int minAbsoluteSumDiff(int[] nums1, int[] nums2) {
        ...
        int diffSum = 0;
        // 拉最大的
        PriorityQueue<Pair> heap = new PriorityQueue<>((o1, o2) -> (o2.diff - o1.diff));
        for (int i = 0; i < nums1.length; i++) {
        ...
        // 从堆中取出最大差
        int maxReduce = Integer.MAX_VALUE;
        int maxIdx = -1;
        int reduce = Integer.MAX_VALUE;
        while (!heap.isEmpty()) {
            Pair currPair = heap.poll();
            int numB = nums2[currPair.idx];
            if (map.get(numB) < maxReduce) {
                maxReduce = map.get(numB);
                maxIdx = currPair.idx;
                reduce = Math.min(reduce, (maxReduce - Math.abs(nums2[maxIdx] - nums1[maxIdx])));
            }
        }
